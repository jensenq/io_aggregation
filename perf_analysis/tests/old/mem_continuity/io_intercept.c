...
void flush_buf(file_buf* fb){
	...
	void* buf_start = &(fb->write_buf);
	void* buf_end =   &(fb->write_buf[fb->curr_size]);
	void* buf_mid =   &(fb->write_buf[(fb->curr_size)/2]);

	void* last = buf_start;
	for(int i=1; i<fb->curr_size; i++){
		if(&fb->write_buf[i] != &fb->write_buf[i-1]+1){
			printf("%p | %p\n", &fb->write_buf[i],  &fb->write_buf[i-1]+1 );
			break;
		}
	}
